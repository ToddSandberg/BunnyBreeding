...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetAxisRaw("Horizontal") != 0 || Input.GetAxisRaw("Vertical") != 0) {
            ...
            if (!(horizontalMovement < 0 && transform.position.x < -8) && !(horizontalMovement > 0 && transform.position.x > 30) && !(verticalMovement > 0 && transform.position.y > 24) && !(verticalMovement < 0 && transform.position.y < -11))
